...
...so I'll put that at the very bottom of this file.  (Currently that'll generate an error, because we haven't enabled jekyll-scholar yet, but we'll do that next.)   The optional argument `--cited` means that it'll only include those references that are actually used in your post.
